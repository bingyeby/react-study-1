...
export default class DataTable extends Component {
  ...
  constructor (props) {
    ...
    let imrows = null
    if (props.tableData != null && props.tableData.rows != null) { imrows = fromJS(props.tableData.rows) } else { imrows = new List() }
  ...
  componentWillReceiveProps (nextProps) {
    ...
    if (typeof nextProps.fetchAction !== 'function') { return }
  ...
  getTableConfigs (tableData) {
    ...
    if (this.props.remotePaging) { total = tableData.total }
  ...
  getColumns (tableData) {
    ...
    && tableData.ids.length === tableData.hds.length) {
      ...
      if (tableData.hiddens) { hiddens = tableData.hiddens }

      for (let i in tableData.ids) {
        let colid = tableData.ids[i]
        if (colid === 'ROWID') { continue }

        if (hiddens != null && hiddens[tableData.ids[i]] === true) { continue }
            ...
            Object.assign(col, {
              sorter: (a, b) => {
                ...
                if (React.isValidElement(va)) {
                  ...
                  if (typeof va === 'object') { va = va.value }
                // console.log(va)
                }
                if (React.isValidElement(vb)) {
                  ...
                  if (typeof vb === 'object') { vb = vb.value }
                // console.log(vb)
                }

                if (va == null && vb == null) { return 0 } else if (va == null && vb != null) { return 1 } else if (vb == null && va != null) { return -1 } else if (typeof va === 'number' && typeof vb === 'number') { return va - vb } else if (typeof va === 'string' && typeof vb === 'string') { return va.localeCompare(vb) }
  ...
  getNeedEditable = (editors) => {
    ...
    if (editors && editors.length > 0) {
      needEditable = editors.map(editor => {
        if (editor === true || editor.editable === true) { return true }
        return false
      })
    }
  ...
  startTimer (_props, _params) {
    ...
    if (_props != null) { fetchAction = _props.fetchAction }
    if (typeof fetchAction !== 'function') { return }

    let refreshInterval = this.props.refreshInterval
    if (_props != null) { refreshInterval = _props.refreshInterval }
    if (typeof refreshInterval !== 'number') { refreshInterval = 0 }

    let queryParams = this.props.queryParams
    if (_props != null) { queryParams = _props.queryParams }

    let params = _params
    if (params == null) { params = this.getParams() }

    if (refreshInterval >= 0) { fetchAction(this.formatPageParams({ ...params, ...queryParams })) }
  ...
  editClick = () => {
    ...
    if (tableData.editors) { editors = tableData.editors }
            ...
            for (let i = 0, l = changerows.length; i < l; i++) {
              for (let key of newrows.get(changerownum[i]).keySeq()) {
                if (newrows.get(changerownum[i]).get(key) !== oldrows.get(changerownum[i]).get(key)) {
                  ...
                  colorrows = colorrows.update(changerownum[i], row => row.update(key,
    ...
    let data = rows.map((srow, rowindex) => {
      ...
      for (let key of tableData.ids) {
        ...
        if (this.props.renderers != null && typeof this.props.renderers[key] === 'function') {
          ...
          // value is object
          if (row[key].value != null) { row[key] = row[key].value } else { row[key] = '' }
      ...
      if (this.props.editable === true && this.state.status === 'edit') {
        for (let j = 0, l = needEditable.length; j < l; j++) {
          if (needEditable[j]) {
            let inputCls = ''
            if (row[tableData.ids[j]] === tableData.rows[rowindex][tableData.ids[j]]) { inputCls = styles.input } else { inputCls = styles['input-edit'] }

            /* 如果使用了 value 表示为受控组件，只能显示 value 的值。如果没有为非受控，使用defaultValue显示初始值*/
            /* 另：如果启用了批量修改同列相同值功能，则需要为受控组件，否则不能实时看到效果*/
            row[tableData.ids[j]] = (<input className={inputCls} value={row[tableData.ids[j]]}
      ...
      if (typeof (this.props.operate) === 'object' && this.props.operate.length > 0) {
        ...
        for (let m = 0, l = this.props.operate.length; m < l; m++) {
          let value
	  if (typeof this.props.operate[m].value !== 'function') { value = this.props.operate[m].value } else {
    value = this.props.operate[m].value(row, tableData, rowindex)
  }

	  let name = value
          if (typeof this.props.operate[m].name === 'string') { name = this.props.operate[m].name } else if (typeof this.props.operate[m].name === 'function') {
            name = this.props.operate[m].name(row, tableData, rowindex)
          }

          if (m >= 1 && value != null) { operate.push(<span className="ant-divider" key={m} />) }

          operate.push(<Tooltip key={name} title={name}>
            <a onClick={this.props.operate[m].event.bind(this, srow, tableData, rowindex,
    ...
    if (!this.props.remotePaging) {
      filteredData = data.filter(row => {
        ...
        for (let v in row) {
          ...
          let tv = typeof row[v]
          // 通用数据过滤
          if ((tv === 'string' || tv === 'number') && (`${row[v]}`).indexOf(this.state.filterValue) >= 0) { return true }
          // 编辑模式下对input进行数据过滤
          if (this.state.status === 'edit' && React.isValidElement(row[v]) &&
          row[v].type === 'input' && row[v].props.value.indexOf(this.state.filterValue) >= 0) { return true }
          // 错误模式下对span进行数据过滤
          if (this.state.status === 'error' && React.isValidElement(row[v]) &&
          row[v].type === 'span' && row[v].props.children.indexOf(this.state.filterValue) >= 0) { return true }
        }
  ...
  handleTableChange = (pagination, filters, sorter) => {
    ...
    if (typeof this.props.fetchAction !== 'function') { return }
    ...
    */
    ...
    if (this.props.refreshInterval > 0) { this.startTimer(this.props, params) } else { this.props.fetchAction(this.formatPageParams({ ...params, ...this.props.queryParams })) }
  }

  formatPageParams (params) {
    let p = { ...params }
    let sort = params.sortField === '' ? '' : params.sortField
    if (sort !== '' && params.sortOrder !== '') { sort += `,${params.sortOrder === 'descend' ? 'desc' : 'asc'}` }
  ...
  render () {
    ...
    let loading = false
    if (this.props.showLoading !== false && (this.props.loading === true || this.state.loading === true)) { loading = true }

    const { columns, dataSource, pagination } = this.getTableConfigs(this.props.tableData)

    let onSearch = null
    if (this.props.showQuickFilter !== false) { onSearch = this.onQuickFilter }

    let updateTime = null
    if (this.props.showUpdateTime !== false) { updateTime = this.props.updateTime }

    let pager = false
    if (this.props.showPager !== false) { pager = pagination }
...
export function getSubmitUrlParams (row, tableData) {
  ...
  if (editors.length > 0) {
    let needSubmit = editors.map(editor => {
      if (editor && editor.submit === true) { return true }
      return false
    })
    ...
    for (let i in needSubmit) {
      if (needSubmit[i] === true) {
        if (!isFirst) { submitStr += '&' } else { isFirst = false }
